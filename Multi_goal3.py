...
df=pd.read_csv('/home/ubuntu/robot5_ws/src/Nox_robot/nox/Coordinates/coordinates.csv')
df_nz=pd.read_csv('/home/ubuntu/robot5_ws/src/Nox_robot/nox/Coordinates/coordinates_nz.csv')
df_table_goal=pd.read_csv('/home/ubuntu/robot5_ws/src/Nox_robot/nox/Coordinates/Table.csv')
...
num=[0]
def Instruction(links):
    for link in links:
        try:
            a=requests.get(link)
            a=a.json()
            #print('3333333333333333333333',a)
            bot_name1=a['bot_name']
            num[0]=a["table_no"]
            t[0]=a['time']
            #print(link)
            if bot_name==bot_name1:
               #print('yes pls')
                if t[0] not in l:
                    print('user1')
                    l.append(t[0])
